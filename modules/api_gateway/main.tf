...
resource "aws_apigatewayv2_integration" "healthcheck_apigw_post" {
...
resource "aws_apigatewayv2_route" "healthcheck_apigw_post" {
  ...
  target    = "integrations/${aws_apigatewayv2_integration.healthcheck_apigw_post.id}"
}

resource "aws_apigatewayv2_integration" "healthcheck_apigw_get" {
  api_id = aws_apigatewayv2_api.healthcheck_apigw.id
...
resource "aws_apigatewayv2_route" "healthcheck_apigw_get" {
  ...
  target    = "integrations/${aws_apigatewayv2_integration.healthcheck_apigw_get.id}"
}